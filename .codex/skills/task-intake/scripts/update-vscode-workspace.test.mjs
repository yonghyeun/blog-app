...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "task-intake-workspace-"));
...
if [[ "$1" == "-C" && "$3" == "rev-parse" && "$4" == "--show-toplevel" ]]; then
  printf '%s\\n' "\${TASK_INTAKE_WORKSPACE_TEST_REPO:?}"
  exit 0
fi
...
execFileSync(process.execPath, [scriptPath], {
  ...
  env: {
    ...
    TASK_INTAKE_WORKSPACE_TEST_REPO: repoRoot,
  },
  encoding: "utf8",
});
...
console.log("task-intake update-vscode-workspace checks passed");
